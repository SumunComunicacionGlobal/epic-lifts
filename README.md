...
Hi. I'm a starter theme called `epic-lifts`. I'm a theme meant for hacking so don't use me as a Parent Theme. Instead try turning me into the next, most awesome, WordPress theme out there. That's what I'm here for.
...
`epic-lifts` requires the following dependencies:
...
To start using all the tools that come with `epic-lifts`  you need to install the necessary Node.js and Composer dependencies :
...
`epic-lifts` comes packed with CLI commands tailored for WordPress theme development :
...
4. Search for `epic-lifts` (in uppercase) to capture constants and replace with: `Nombre_proyecto`.
